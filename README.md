...
![Пример работы сервиса](assets/Predict_price.PNG)

Сервис развёрнут на локальной виртуальной машине, доступ к нему осуществляется через сетевой мост 192.168.0.202. Внешнего доступа нет, но инфраструктура полностью идентична production-окружению.
...
![Пример успешного CI/CD пайплайна](assets/pipline.png)
...
```
upstream uber_upstream {
    server uber-app-blue:32000 max_fails=5 fail_timeout=5s;
}
```
...
upstream ml_upstream {
    server uber-app-green:32000 max_fails=5 fail_timeout=5s;
}
```
...
I. `deploy.sh` анализирует файл `nginx/conf.d/upstream.conf`, чтобы определить, какой цвет в данный момент активен. Если активен `blue`, новый выкат пойдёт в слот `green`, и наоборот. Это гарантирует, что обновляется именно резервный цвет, а не тот, через который сейчас идёт трафик.
...
![Пример Rollback-а](assets/container.PNG)

---

![Пример Rollback-а](assets/container_2.PNG)
...
-d '{"data":[{"pickup_latitude":21,15,"pickup_longitude":-25,30,"dropoff_latitude":21,20,"dropoff_longitude":-5,2,"passenger_count":2}]}'
```
