...
if grep -E '^[[:space:]]*server[[:space:]]+uber-app-blue' "${NGINX_CONF_DIR}/upstream.conf" >/dev/null 2>&1; then
  ACTIVE_COLOR="blue"
  NEW_COLOR="green"
  OLD_COLOR="blue"
else
...
if ! docker ps --format '{{.Names}}' | grep -q '^uber-proxy$'; then
  echo "Proxy container not found, starting it..."
  docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" up -d proxy
else
  echo "Proxy container uber-proxy already running, will not recreate."
fi

# 4. Чистим контейнер нового цвета (если был)
echo "Cleaning stale container for new color: $NEW_COLOR"
docker rm -f "uber-app-${NEW_COLOR}" >/dev/null 2>&1 || true

# 5. Подтягиваем и поднимаем новую ревизию для нового цвета
echo "Pulling new image for uber-app-${NEW_COLOR}..."
docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" pull "app_${NEW_COLOR}"

echo "Starting new uber-app-${NEW_COLOR}..."
docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" up -d "app_${NEW_COLOR}"

# 6. Ждём HEALTHCHECK нового контейнера
echo "Waiting for uber-app-${NEW_COLOR} to be healthy..."
ATTEMPTS=20
SLEEP_SEC=3

for i in $(seq 1 $ATTEMPTS); do
  STATUS=$(docker inspect --format='{{json .State.Health.Status}}' "uber-app-${NEW_COLOR}" 2>/dev/null || echo "\"starting\"")
  echo "Health status: $STATUS"
  if [ "$STATUS" = "\"healthy\"" ]; then
    echo "✅ New version uber-app-${NEW_COLOR} is healthy!"
    break
  fi
  sleep "$SLEEP_SEC"
done
...
if [ "$STATUS" != "\"healthy\"" ]; then
  echo "❌ ERROR: New version uber-app-${NEW_COLOR} did not become healthy."
...
docker exec uber-proxy nginx -s reload
...
echo "Stopping old version: uber-app-${OLD_COLOR}"
